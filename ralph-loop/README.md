...
Ralph Loop автоматизирует цикл реализации: **Реализация → Ревью → Коммит**. Качество кода обеспечивается запуском специализированных review-агентов (субагентов) перед каждым коммитом.
...
```bash
# Генерация задач из плана
./ralph-loop/scripts/generate-tasks.sh --plan-path features/001-auth/plan.md

# Запуск Ralph Loop
./ralph-loop/scripts/ralph_loop.sh --tasks-path features/001-auth/tasks.md

# Без ревью (только для hotfix)
./ralph-loop/scripts/ralph_loop.sh --tasks-path features/001-auth/tasks.md --no-review

# Подробный вывод
./ralph-loop/scripts/ralph_loop.sh --tasks-path features/001-auth/tasks.md --verbose
```
...
│   └── generate-tasks.sh  # Генератор задач из плана
└── prompts/
...
2. Ralph Loop останавливается с понятным уведомлением
3. Пользователь запускает Planning Agent для создания fix-задач
4. Ralph Loop продолжает после исправления
...
- **generate-tasks.sh** — генерация задач из plan.md

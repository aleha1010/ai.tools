# Task Loop

[![License: MIT](https://img.shields.io/badge/License-MIT-yellow.svg)](https://opensource.org/licenses/MIT)

Автономный TDD-оркестратор с мульти-агентным ревью перед коммитом.

## Обзор

Task Loop автоматизирует цикл реализации: **Реализация → Ревью → Коммит**. Качество кода обеспечивается запуском специализированных review-агентов (субагентов) перед каждым коммитом.

**Ключевые возможности v2.0:**
- **SpecKit независимость** — не требует внешних зависимостей
- **Dependencies проверка** — задачи выполняются в правильном порядке
- **Escalation Protocol** — обработка непредвиденных проблем
- **Изоляция агента** — агент не может влиять на список задач

```
┌─────────────────────────────────────────────────────┐
│                    Task Loop v2.0                   │
│                                                      │
│  ┌──────────┐    ┌──────────┐    ┌──────────┐       │
│  │Реализация│───▶│  Ревью   │───▶│  Коммит  │       │
│  │  Агент   │    │ (субагенты)│    │          │       │
│  └──────────┘    └──────────┘    └──────────┘       │
│       │               │                              │
│       │          ┌────┴────┐                        │
│       │          │         │                        │
│       │     APPROVED  REJECTED                       │
│       │          │         │                         │
│       │          ▼         ▼                         │
│       │       Коммит   Исправление                   │
│       │                   │                          │
│       │              ESCALATION                       │
│       │                   │                          │
│       │            Pause & Notify                    │
└──────┴──────────────────────────────────────────────┘
```

## Возможности

- **Двухфазный подход**: Реализация отделена от коммита
- **Мульти-агентное ревью**: специализированные ревьюеры запускаются как субагенты параллельно через task tool
- **State Machine**: Явное отслеживание состояния с поддержкой восстановления
- **Circuit Breaker**: Остановка после 3 последовательных неудач
- **Структурированный вывод**: JSON-протокол коммуникации
- **Dependencies проверка**: Задачи выполняются в правильном порядке
- **Escalation Protocol**: Обработка непредвиденных проблем с handoff
- **SpecKit независимость**: Не требует внешних зависимостей

## Структура проекта v2.0

```
features/
  001-auth/
    plan.md           → архитектура фичи
    tasks.md          → индекс задач (порядок = последовательность)
    tasks/
      T001.md         → спецификация задачи с YAML frontmatter
      T002.md
    progress.md       → история выполнения
```

## Быстрый старт

```bash
# Запуск Task Loop
./task-loop/scripts/task_loop.sh --tasks-path features/001-auth/tasks.md

# Без ревью (только для hotfix)
./task-loop/scripts/task_loop.sh --tasks-path features/001-auth/tasks.md --no-review

# Подробный вывод
./task-loop/scripts/task_loop.sh --tasks-path features/001-auth/tasks.md --verbose
```

## Требования

| Зависимость | Версия | Назначение |
|-------------|--------|------------|
| `kilo` | любая | AI agent CLI |
| `jq` | >=1.6 | Парсинг JSON |
| `git` | >=2.0 | Контроль версий |

## Архитектура

### Структура директорий

```
task-loop/
├── README.md
├── scripts/
│   └── task_loop.sh        # Bash-оркестратор
└── prompts/
    ├── task-iterate.md     # Prompt агента реализации
    └── task-review.md      # Prompt агента ревью
```

### Формат задачи (tasks/T001.md)

```yaml
---
id: T001
dependencies: [T000]
---
# T001: Название задачи

## Context
Связь с планом, контекст выполнения

## Test Specification (RED Phase)

### Test Cases
| ID | Scenario | Given | When | Then | Type |
|----|----------|-------|------|------|------|
| T1 | Happy path | valid input | call method | returns success | unit |

## Implementation Specification (GREEN Phase)
Что реализовать

## Acceptance Criteria
- [ ] All test cases pass
- [ ] Coverage ≥ 80%
```

### Dependencies проверка

Orchestrator проверяет `dependencies` в YAML frontmatter:
- Если dependencies не выполнены → задача BLOCKED
- Если все dependencies `[x]` → задача EXECUTABLE
- Если все задачи BLOCKED → graceful exit с ошибкой

### Escalation Protocol

При обнаружении непредвиденной проблемы:

1. Агент создаёт `.escalation_handoff.md`
2. Task Loop останавливается с понятным уведомлением
3. Пользователь запускает Planning Agent для создания fix-задач
4. Task Loop продолжает после исправления

**Триггеры для escalation:**
- Неописанная зависимость
- Противоречивые требования
- Технический блокер
- Scope creep

### State Machine

```
         ┌──────────────────────────────────────┐
         │                                      │
         ▼                                      │
       IDLE ──────▶ IMPLEMENTING ──────▶ REVIEWING
         │                │                   │
         │                │                   │
         │                ▼                   ▼
         │            FAILED             ┌────┴────┐
         │                │              │         │
         │                │         APPROVED  REJECTED
         │                │              │         │
         │                │              ▼         │
         │                │         COMMITTING     │
         │                │              │         │
         │                │              ▼         │
         │                │           IDLE ◀──────┘
         │                │                          
         ▼                ▼                          
      COMPLETE         (circuit breaker)            
```

### Фазы

#### Фаза 1: Реализация

Агент (`task-iterate.md`):
1. Читает `tasks.md`, `progress.md`, `plan.md`
2. Реализует одну user story
3. Помечает задачи как `[x]`
4. Выводит: `{"signal": "USER_STORY_COMPLETE"}`

#### Фаза 2: Review Gate

Агент (`task-review.md`):
1. Определяет выполненные задачи
2. Запускает ревьюеров как субагентов через `task` tool (параллельно):

| Ревьюер | Фокус |
|---------|-------|
| `review-analyst` | Бизнес-требования, acceptance criteria |
| `review-security` | OWASP Top 10, SQL injection, XSS |
| `review-architect-backend` | DI-паттерны, разделение слоёв |
| `review-performance` | N+1 queries, memory leaks, async |
| `review-tester` | AAA pattern, coverage, mocks |

3. Агрегирует результаты
4. Выводит: `{"signal": "REVIEW_APPROVED"}` или `{"signal": "REVIEW_REJECTED"}`

#### Фаза 3: Коммит

Оркестратор (`task_loop.sh`):
1. Создаёт git commit (только после APPROVED)
2. Включает статус ревью в сообщение коммита
3. Обновляет progress log

## Конфигурация

### Параметры командной строки

| Параметр | Обязательный | По умолчанию | Описание |
|----------|--------------|--------------|----------|
| `--tasks-path PATH` | Да | — | Путь к tasks.md |
| `--max-iterations N` | Нет | 50 | Максимум итераций (1-1000) |
| `--no-review` | Нет | false | Отключить Review Gate |
| `--verbose` | Нет | false | Подробный вывод |
| `--working-directory DIR` | Нет | . | Рабочая директория |

### Переменные окружения

| Переменная | Описание |
|------------|----------|
| `KILO_CMD` | Переопределить команду kilo (для тестов) |

## JSON-протокол

### Реализация завершена

```json
{
  "signal": "USER_STORY_COMPLETE",
  "tasks_completed": ["T001", "T002"],
  "files_changed": ["src/Service.cs", "tests/ServiceTests.cs"]
}
```

### Ревью пройдено

```json
{
  "signal": "REVIEW_APPROVED",
  "tasks": ["T001", "T002"],
  "verdicts": {
    "analyst": "APPROVED",
    "security": "APPROVED",
    "architect": "CONDITIONALLY_APPROVED",
    "performance": "APPROVED",
    "tester": "APPROVED"
  }
}
```

### Ревью отклонено

```json
{
  "signal": "REVIEW_REJECTED",
  "reviewer": "security",
  "issues": [
    "SQL injection в методе GetParticipants",
    "Отсутствует валидация входных данных"
  ]
}
```

### Все задачи выполнены

```json
{
  "signal": "COMPLETE"
}
```

## Генерируемые файлы

| Файл | Назначение | Права |
|------|------------|-------|
| `.task_loop_state.json` | State machine tracking | 600 |
| `.task_loop.log` | Лог выполнения | 600 |
| `.task_loop_rejection_context.md` | Контекст для исправления REJECTED | 600 |

### Формат Progress Log

Дописывается в `specs/*/progress.md`:

```markdown
## Review - 2026-06-12T12:30:00Z

**Задачи**: T001, T002

### Результаты ревью

| Ревьюер | Вердикт | CHK | Проблемы |
|---------|---------|-----|----------|
| analyst | APPROVED | CHK001 | 0 HIGH, 0 MEDIUM, 0 LOW |
| security | APPROVED | CHK002 | 0 HIGH, 0 MEDIUM, 0 LOW |
| architect | CONDITIONALLY_APPROVED | CHK003 | 0 HIGH, 1 MEDIUM, 0 LOW |
| performance | APPROVED | CHK004 | 0 HIGH, 0 MEDIUM, 0 LOW |
| tester | APPROVED | CHK005 | 0 HIGH, 0 MEDIUM, 0 LOW |

### Checklist Items

- [x] CHK001: Задачи T001,T002 соответствуют требованиям
- [x] CHK002: Задачи T001,T002 безопасны
- [x] CHK003: Задачи T001,T002 архитектура корректна
- [x] CHK004: Задачи T001,T002 производительность в норме
- [x] CHK005: Задачи T001,T002 тесты качественные

### Technical Debt

Нет

### Решение

APPROVED: Все ревьюеры прошли — коммит разрешён
```

## Обработка ошибок

### Circuit Breaker

- **3 последовательных неудачи реализации** → Остановка
- **2 последовательных неудачи ревью** → Остановка
- **Exponential backoff**: 2^n секунд (макс 60с)

### Восстановление

При REJECTED:
1. Контекст сохраняется в `.task_loop_rejection_context.md`
2. Задачи остаются помеченными `[x]`
3. Следующая итерация читает контекст и исправляет проблемы

## Безопасность

### Меры защиты

| Угроза | Защита |
|--------|--------|
| Command injection | Используется `git commit -F file` вместо `-m` |
| JSON injection | Whitelist-валидация через `jq select()` |
| Path traversal | Валидация `realpath` относительно корня проекта |
| Права файлов | `chmod 600` для log/state файлов |

### Исправленные HIGH-находки

Все HIGH-находки из security review исправлены:
- ✅ Command injection в git commit
- ✅ Непроверенный JSON parsing
- ✅ Проблемы с правами файлов

## Известные ограничения

| Ограничение | Влияние | Митигация |
|-------------|---------|-----------|
| Нет тестов для bash-скрипта | Среднее | Использовать bats для критических путей |
| Prompts не тестируются автоматически | Среднее | Integration tests с mock kilo |
| Sleep замедляет тесты | Низкое | Переопределить `SLEEP_CMD` в тестах |

## Changelog

### [2.1.0] - 2026-06-28

#### Breaking Changes
- Renamed: `ralph-loop` → `task-loop` (directory, scripts, prompts, state files)
- Removed: lock mechanism (`--force` flag, `LOCK_FILE`, `FORCE_LOCK`)
- Removed: `migrate_old_files()` function (v1→v2 bridge, no longer needed)
- State files: `.ralph_*` → `.task_loop_*` — NOT migrated, loop starts fresh
- Removed: dead documentation (`RALPH_TEST_MODE`, `generate-tasks.sh` references)
- Fixed: contract mismatch `.ralph_review_results.md` (plural) vs `.ralph_review_result.md` (singular)

#### Migration Guide
- Before: `./ralph-loop/scripts/ralph_loop.sh --tasks-path ...`
- After: `./task-loop/scripts/task_loop.sh --tasks-path ...`
- Old `.ralph_state.json` etc. are ignored — loop restarts from task 1
- `--force` flag removed — no replacement (ensure no parallel runs)

## ⚠️ Important: No parallel runs

Task Loop does not use file locking. Running two instances in parallel
will cause race conditions on state files (`.task_loop_state.json`).
Run one instance at a time.

## Migration from ralph-loop (v2.0)

If upgrading from `ralph-loop` v2.0:
- Rename paths: `./ralph-loop/` → `./task-loop/`, `ralph_loop.sh` → `task_loop.sh`
- State files (`.ralph_*`) are NOT migrated — loop starts fresh
- `--force` flag removed — do not run multiple instances in parallel

### [2.0.0] - 2026-06-24

#### Добавлено
- **SpecKit независимость** — убрана зависимость от `.specify/scripts/`
- **Dependencies проверка** — задачи выполняются в правильном порядке
- **Escalation Protocol** — обработка непредвиденных проблем
- **Изоляция агента** — агент получает только файл задачи, не может влиять на список
- **generate-tasks.sh** — генерация задач из plan.md (deprecated: use task-generator skill instead)
- **Кэширование** — status_cache и frontmatter_cache для производительности

#### Изменено
- Структура: `specs/` → `features/` с поддиректорией `tasks/`
- Агент получает только `tasks/T001.md`, не весь `tasks.md`
- Orchestrator помечает `[x]`, не агент
- State Machine: добавлены состояния ESCALATION, RESUME

#### Удалено
- Зависимость от `.specify/scripts/bash/check-prerequisites.sh`
- Доступ агента к tasks.md, progress.md, plan.md

### [1.1.0] - 2026-06-12

#### Добавлено
- Структурированный JSON output protocol
- State machine с `.task_loop_state.json`
- Файл контекста отклонения для восстановления
- Поддержка параллельного выполнения ревью
- Проверка зависимостей `jq` и `git`

#### Изменено
- Review Gate: signal-based → JSON-based протокол
- Подсчёт задач: исправлен баг подсчёта всех задач вместо новых
- Git commit: теперь использует file-based message (security fix)

#### Исправлено
- Command injection уязвимость в git commit
- JSON parsing без валидации
- Некорректный подсчёт выполненных задач

#### Безопасность
- Добавлен `chmod 600` для log и state файлов
- Whitelist-валидация для JSON signals
- File-based commit messages

### [1.0.0] - 2026-06-10

#### Добавлено
- Начальная реализация
- Двухфазный подход (реализация → ревью → коммит)
- Мульти-агентное Review Gate
- Circuit Breaker
- Exponential backoff

## Установка в проект

```bash
# Копирование в проект
cp -r /Users/alexey/Workspace/ai.tools/task-loop ./

# Запуск
./task-loop/scripts/task_loop.sh --tasks-path specs/001-feature/tasks.md
```

## Лицензия

MIT License — см. файл LICENSE.
